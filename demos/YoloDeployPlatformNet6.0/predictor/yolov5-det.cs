...
{
    public class YOLOv5Det : YOLO
    {
        public YOLOv5Det(string model_path, string device, int categ_nums, float det_thresh, float det_nms_thresh, int input_size)
            : base(model_path, device, categ_nums, det_thresh, det_nms_thresh, new int[] { 1, 3, input_size, input_size }, 
                  new List<string> { "images" }, new List<int[]> { new int[] { 1, 25200, 5 + categ_nums } }, new List<string> { "output" })
        {
        }
        protected override BaseResult postprocess(List<float[]> results)
        {
            ...
            for (int i = 0; i < result_data.Rows; i++)
            {
                ...
                if (max_score > this.m_det_thresh)
                {
