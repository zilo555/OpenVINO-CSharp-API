...
{
    public class Yolov5Seg : Predictor
    {
        ...
        public Yolov5Seg(Yolov5SegConfig config)
        ...
        public List<SegResult> process_result(float[] detect, float[] proto, int batch)
        {
            ...
            for (int b = 0; b < batch; ++b)
            {
                ...
                for (int i = 0; i < detect_data.Rows; i++)
                {
                    ...
                    Mat classes_scores = new Mat(detect_data, new Rect(5, i, m_categ_nums, 1));//GetArray(i, 5, classes_scores);
